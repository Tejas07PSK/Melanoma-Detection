        ...
        for idx_pair, feat in zip(ftup, feats):
                ...
                for mdl, title in zip(classifiers, titles):
                        ...
                        if (plot_index == (len(ftup) * len(classifiers))):
                            (obj).legend(handles=[Pchs.Patch(color='red', label='MALIGNANT'), Pchs.Patch(color='yellow', label='BENIGN'), Pchs.Patch(color='green', label='NEGATIVE')], loc='lower right', bbox_to_anchor=(-0.3, -0.3))
