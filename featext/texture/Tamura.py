...
class TamFeat(object):
    ...
    def __init__(self, img):
        ...
        self.img_hor_x = cv2.filter2D(cv2.GaussianBlur(img, (3, 3), 0), -1, np.array([[1,1,1],[0,0,0],[-1,-1,-1]], dtype=np.int16))
        self.img_vert_y = cv2.filter2D(cv2.GaussianBlur(img, (3, 3), 0), -1, np.array([[-1,0,1],[-1,0,1],[-1,0,1]], dtype=np.int16))
